...
def recalc(filename, timeout=30, force=False):
    ...
    # LibreOffice's macro URI does not return reliably through the macOS
    # headless launcher used by General Agent. A same-format conversion in an
    # isolated output directory performs the same calculate-and-save operation
    # and returns promptly while preserving the source on failure.
    if platform.system() == "Darwin":
        with tempfile.TemporaryDirectory(
            prefix="recalc-lo-convert-", ignore_cleanup_errors=True
        ) as output_dir:
            return _recalc_by_conversion(
                filename, abs_path, timeout, Path(output_dir)
            )
...
def _recalc_by_conversion(filename, abs_path, timeout, output_dir: Path):
    extension = Path(filename).suffix.lower().lstrip(".")
    if extension not in {"xlsx", "xlsm", "xltx"}:
        return {"error": f"Unsupported workbook extension for recalculation: .{extension}"}
    ...
    try:
        ...
        return {
            "error": (
                f"LibreOffice timed out after {timeout}s; formulas were NOT "
                "recalculated. Re-run with a longer timeout."
            )
        }
    except FileNotFoundError:
    ...
    if result.returncode != 0 or not converted.is_file():
        ...
        if not detail:
            detail = f"soffice exited {result.returncode} without an output file"
        return {"error": f"LibreOffice failed to recalculate: {detail}"}

    os.replace(converted, abs_path)
    return _inspect_recalculated_workbook(filename)
...
def _inspect_recalculated_workbook(filename):
    try:
