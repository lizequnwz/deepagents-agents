...
@pytest.mark.asyncio
async def test_v3_events_project_root_tools_subagents_plans_and_usage(settings) -> None:
    graph = FakeGraph()
    ...
    assert tool_start.agent == "general-purpose"
    assert tool_start.data["input"]["command"] == "python work.py"
    tool_finish = next(event for event in run.events if event.kind == "tool_finished")
    assert tool_finish.data["output"]["exit_code"] == 0
    assert tool_finish.data["output"]["output"].startswith("ok")
    assert run.diagnostics.tokens.total_tokens == 10
    assert run.diagnostics.model_calls_missing_usage == 1
    assert run.diagnostics.token_usage_partial is True
    assert {usage.agent for usage in run.diagnostics.agents} == {
        "general-agent",
        "general-purpose",
    }
