...
def reduce_live_events(state: dict[str, Any], events: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for event in events:
        ...
        if kind == "plan_updated" and event.get("agent") == "general-agent":
...
def render_live_run(client: AgentAPIClient, run: dict[str, Any], state: dict[str, Any]) -> None:
    label = _latest_activity_label(state) or "General Agent is working…"
...
def _render_tool(event: dict[str, Any]) -> None:
    ...
    expanded = name == "execute"
    with st.expander(
        event.get("label") or name,
        icon=":material/code:" if name == "execute" else ":material/build:",
        expanded=expanded,
    ):
        tool_input = data.get("input")
        if name == "execute" and isinstance(tool_input, dict):
            st.code(str(tool_input.get("command") or tool_input), language="bash")
        elif tool_input is not None:
            st.json(tool_input)
        if "output" in data and data.get("output") not in (None, ""):
            output = data["output"]
            if name == "execute" and isinstance(output, dict):
                st.code(str(output.get("output") or ""), language="text")
                exit_code = output.get("exit_code")
                truncated = bool(output.get("truncated"))
                details = f"Exit code {exit_code}" if exit_code is not None else "Exit status unavailable"
                if truncated:
...
def _agent_label(agent: Any) -> str:
    value = str(agent or "general-agent")
    if value == "general-agent":
        return "Deep Agent"
