...
To use a local file, open **Upload a file**, choose CSV or Parquet, and click
**Review file**. Check suggested column types, optionally declare what one row
represents and the columns that uniquely identify it, then confirm. Analysis is
unavailable until confirmation. The review shows ten sample rows; conversion and
...
Partial findings say what remains unresolved. Retry report preserves finished
