(() => {
  ...
  const delivery={local:['Local preview','Built HTML + a separate saved data file','A local web server serves the app and its matching data file. The browser displays saved results; the host runs any fresh source queries. This teaching presentation is different: its data is embedded, so index.html opens directly.'],offline:['Portable offline HTML','Reviewed snapshot embedded in the artifact','Rendering travels with the file. Fresh source reads, host-assisted conversion, and scheduling still need their capabilities.'],sites:['Sites publication','Exact compiled UI + hosted snapshot and presentation','The delivery workflow packages the current app, transfers bound assets, and checks readback while preserving Site access and user state.']};
