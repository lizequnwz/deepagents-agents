...
const pdf=await getDocument({data:new Uint8Array(fs.readFileSync(path.join(here,'learning-layer-print.pdf'))),useSystemFonts:true}).promise;
const pages=[];
...
for(let i=1;i<=pdf.numPages;i++){
  ...
  if(i===1||str.includes('The plugin defines the job.')){
