...
<main>${sceneHtml}<section id="research" class="research" aria-labelledby="research-title"><div class="research-inner"><span class="eyebrow">Research appendix / optional depth</span><h2 id="research-title">Keep learning below the surface.</h2><p>The complete original review is embedded here, including architecture findings, source references, test receipts, limitations, and improvement proposals. Explore it without leaving this file.</p><div class="research-actions"><button id="open-review" aria-expanded="false" aria-controls="review-viewer">Open full technical review</button><button id="all-sources">Browse all source references</button><button id="download-review">Download original HTML</button><button id="download-markdown">Download original Markdown</button><a class="button-link" href="PRESENTER_GUIDE.md" target="_blank" rel="noopener">Presenter guide · separate tab ↗</a><a class="button-link" href="PRESENTER_SCRIPT.md" target="_blank" rel="noopener">Word-by-word script · separate tab ↗</a></div><small>Presenter Markdown links need the companion files beside index.html. Printing this lesson includes the currently selected interactive views; use the original review for the complete technical document. Teaching layer → implementation detail → original evidence. The learning exercises are illustrative simulations, not a running Data app.</small><div class="review-viewer" id="review-viewer" hidden><iframe id="review-frame" title="Complete original technical review" sandbox="allow-scripts allow-downloads"></iframe></div><div class="footnote">Based on installed Codex Data 1.0.8 and the standalone review of 14 September 2026. Architectural interpretations and adoption lessons are separated from implementation facts in the deep dives. No live connector, Sites identity flow, or cloud refresh was executed for this lesson. The installed plugin and original review are unchanged.</div></div></section></main>
...
const coreMinutes=content.scenes.reduce((total,scene)=>total+scene.duration,0);
...
The scene budgets total **${coreMinutes} minutes** for a selective talk using these notes. Allow **30–35 minutes** for the fuller word-by-word script and its interactions, before audience questions or live Codex demos. These are planning estimates, not a timed rehearsal. Demonstrations are optional: replace discussion time or add 4–8 minutes. All numbers and advisor examples are visibly labelled teaching fixtures. Never present them as plugin test output or business findings.
...
**Open scene:** [${s.nav}](index.html#${s.id})

**Screen heading:** ${s.title.replaceAll('<br>',' ')}
