...
If you need to fill out a PDF form, first check to see if the PDF has fillable form fields. Run this script from this file's directory:
 `python "$GENERAL_AGENT_SKILLS_DIR/pdf/scripts/check_fillable_fields.py" <file.pdf>`, and depending on the result go to either the "Fillable fields" or "Non-fillable fields" and follow those instructions.

# Fillable fields
If the PDF has fillable form fields:
- Run this script from this file's directory: `python "$GENERAL_AGENT_SKILLS_DIR/pdf/scripts/extract_form_field_info.py" <input.pdf> <field_info.json>`. It will create a JSON file with a list of fields in this format:
```
[
  {
...
`python "$GENERAL_AGENT_SKILLS_DIR/pdf/scripts/convert_pdf_to_images.py" <file.pdf> <output_directory>`
...
`python "$GENERAL_AGENT_SKILLS_DIR/pdf/scripts/fill_fillable_fields.py" <input pdf> <field_values.json> <output pdf>`
...
`python "$GENERAL_AGENT_SKILLS_DIR/pdf/scripts/extract_form_structure.py" <input.pdf> form_structure.json`
...
`python "$GENERAL_AGENT_SKILLS_DIR/pdf/scripts/check_bounding_boxes.py" fields.json`
...
`python "$GENERAL_AGENT_SKILLS_DIR/pdf/scripts/convert_pdf_to_images.py" <input.pdf> <images_dir/>`
...
`python "$GENERAL_AGENT_SKILLS_DIR/pdf/scripts/fill_pdf_form_with_annotations.py" <input.pdf> fields.json <output.pdf>`

## Step 4: Verify Output

Convert the filled PDF to images and verify text placement:
`python "$GENERAL_AGENT_SKILLS_DIR/pdf/scripts/convert_pdf_to_images.py" <output.pdf> <verify_images/>`
