...
The skill directory is read-only and shell commands start in the current chat.
Expand every relative `scripts/...` path below to
`$GENERAL_AGENT_SKILLS_DIR/pptx/scripts/...`; keep builders, unpacked working
files, rendered previews, and outputs in the current chat or
`$GENERAL_AGENT_TEMP_DIR`.
...
Try `require('pptxgenjs')` before installing anything. If it is unavailable,
install it into the isolated per-user Node directory with `npm install --prefix
"$GENERAL_AGENT_NODE_PACKAGE_DIR" pptxgenjs`; `NODE_PATH` is already configured
for CommonJS resolution. The model knows the API; these are the footguns:
...
- **Icons:** render `react-icons` to SVG (`ReactDOMServer.renderToStaticMarkup`), rasterize with `sharp` at ≥256px, and insert via `addImage({ data: "image/png;base64," + buf.toString("base64") })` — the `image/png;base64,` prefix is required. If these packages are missing, install them only with `npm install --prefix "$GENERAL_AGENT_NODE_PACKAGE_DIR" react-icons react react-dom sharp`.
...
`pptxgenjs` (npm; install into `$GENERAL_AGENT_NODE_PACKAGE_DIR` only if needed) · `markitdown[pptx]`, `Pillow`, `defusedxml`, `lxml` (pip — text dump, thumbnail, clean, validate) · LibreOffice (`soffice`, auto-configured for sandboxed environments via `scripts/office/soffice.py`) · `pdftoppm` (Poppler)
