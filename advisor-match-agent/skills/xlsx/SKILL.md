...
`$GENERAL_AGENT_SKILLS_DIR/xlsx/scripts/...`; keep builders, temporary files,
and outputs in the current chat or `$GENERAL_AGENT_TEMP_DIR`. If a declared
Python dependency is unexpectedly missing, install it only with `python -m pip
install --target "$GENERAL_AGENT_PACKAGE_DIR" ...`.
...
```bash
python "$GENERAL_AGENT_SKILLS_DIR/xlsx/scripts/recalc.py" output.xlsx [timeout_seconds]   # default 30
```
