...
The match tool and later exact-CRD proposals resolve the opaque ID internally, verify its path, hash, schema, and row count, and reuse the same attachment snapshot across turns. Errors are explicit for an empty or oversized source, invalid schema, blank trimmed master CRDs, missing snapshot, cross-corporation access, or integrity failure. Duplicate trimmed master CRDs return a controlled authoritative-source blocker with bounded CRD occurrence counts; they never create a completed snapshot, match session, or workbook. Partial `.building` snapshots are never registered or returned.
