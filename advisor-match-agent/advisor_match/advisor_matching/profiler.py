"""Bounded raw-table profiling for column-mapping interpretation."""
...
def inspect_advisor_upload(source: InMemoryFile, settings: Settings) -> dict[str, Any]:
    ...
    try:
        if suffix == ".csv":
            frames = [(None, _read_csv(source.content, settings.max_inspect_rows + 1))]
            total_sheets = 1
        else:
...
def _read_csv(content: bytes, nrows: int) -> pd.DataFrame:
