...
from advisor_match.advisor_matching.input_loader import input_mapping_fingerprint
...
def build_match_workbook(
    *,
    ...
    _write_summary(
        ...
        firm_override_rows=firm_override_rows,
    )
    output = BytesIO()
    ...
    *,
    ...
    try:
        ...
        if expected_counts is not None and actual_counts != expected_counts:
            raise ValueError(
                "Workbook status counts do not match the structured decisions."
            )
...
def _write_summary(
    ...
    *,
    ...
    values: list[tuple[str, object]] = [
        ...
        ("Reference Source", reference.source_kind),
        ("Reference Schema Version", reference.schema_version),
        ("Reference Query ID", reference.query_id or ""),
    ...
    values.extend(
        [
            ("Firm Resolution", firm_resolution),
            ("User-Supplied Firm", all_rows_firm or ""),
            ("Rows With Firm Override", firm_override_rows),
        ]
    )
...
def _review_row(
    ...
    *,
    ...
    return [
        ...
        "Yes" if decision.candidates_truncated else "No",
